...
- Session 生命周期和标题生成。
- 上下文剪枝、摘要与压缩。
- Memory、Todo、Delegation（子任务委派）、Offer Choice 等高级能力。
...
**已实现**：OpenTUI、主 Agent Loop、工具执行、Session、上下文压缩、L1 Memory、L2 历史会话检索（session_search）、图片识别（多模态附件与视觉模型指定）、Todo、子任务委派（delegate + exploring/writing 子代理 + 钻取视图）、Offer Choice（交互式选项）、Provider/Model 管理和本地存储。

**部分实现**：委派子会话的落盘、resume 与后台模式（当前子代理为内存态，随工具调用结束即释放）。

**设计中或占位**：L3/L4 Memory、Sandbox（沙箱）、Scheduler（定时任务）、QQ 与其他消息入口。
