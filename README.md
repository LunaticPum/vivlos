...
9. **任务规划**：提供结构化 Todo List（待办清单）、状态推进和侧栏查看，并支持交互式选项确认。
10. **本地持久化**：Provider、Model、凭证和自定义服务配置保存在本地 SQLite（嵌入式数据库）；Session（会话）、Memory 与 Todo 保存在当前工作目录。
11. **运行时控制**：提供重试、最大轮次、Bash 权限和压缩策略等配置边界。
