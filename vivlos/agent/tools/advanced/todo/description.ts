...
export const descriptions = {
	...
	write: "创建当前 Session 中尚不存在的 Todo List。items 必须直接传完整、按执行顺序排列的 JSON 数组，不要传字符串；所有 Item 初始为 pending。",
	replace: "完整替换当前 Session 已存在的 Todo List。items 必须直接传新的完整 JSON 数组，不要传字符串；所有 Item 重新初始化为 pending。",
